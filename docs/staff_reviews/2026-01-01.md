...
- Audit durability: enqueue depth, drop count, persist failures, and time-to-persist. `pkg/platform/audit/publisher/publisher.go`.
...
- Audit durability guarantees (drop behavior, backpressure, storage backend). `pkg/platform/audit/publisher/publisher.go`.

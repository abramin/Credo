...
// TestConcurrentFailureRecording verifies that concurrent RecordFailure calls
	...
	for _, id := range ids {
		for j := 0; j < failuresPerIdentifier; j++ {
			...
			go func(identifier string) {
				defer wg.Done()
				_, _ = s.store.RecordFailure(ctx, identifier)
			}(id)
		}
	}
	...
	for i := 0; i < goroutines; i++ {
		...
		go func(idx int) {
			...
			if idx%10 == 0 {
				...
				total, _ := s.store.ResetDailyFailures(ctx)
				resetTotal.Add(int32(total))
			} else {
				// Continue recording failures on recent identifiers
				_, _ = s.store.RecordFailure(ctx, recentIDs[idx%5])
			}
		}(i)
	}
...
// TestIsLockedConcurrency verifies concurrent IsLocked checks don't cause issues.
func (s *PostgresStoreSuite) TestIsLockedConcurrency() {
	ctx := context.Background()
	identifier := "user:" + uuid.NewString()
	...
	s.Equal(int32(0), errors.Load(), "no errors expected")
	s.Equal(int32(goroutines), lockedCount.Load(), "all checks should see locked state")
}
...
func (s *PostgresStoreSuite) TestResetFailureCountTransaction() {
	...
	cfg.WindowDuration = 1 * time.Second // Short window for testing
	store := authlockout.NewPostgres(s.postgres.DB, &cfg)
	...
	for i := 0; i < 5; i++ {
		...
		record := &models.AuthLockout{
			...
			FailureCount:  10,
			DailyFailures: 10,
			LastFailureAt: time.Now().Add(-5 * time.Second), // Older than window
		}
		err := store.Update(ctx, record)
		s.Require().NoError(err)
	}

	// Wait for records to age past window
	time.Sleep(2 * time.Second)

	// Reset should return total failures
	total, err := store.ResetFailureCount(ctx)
	s.Require().NoError(err)
